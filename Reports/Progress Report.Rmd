...
```{r,echo=FALSE}
upreg <- read.delim("../Results/DE_Upregulated.tsv")
head(knitr::kable(upreg), 10)
```

The following is a table showing the downregulated genes:
```{r,echo=FALSE}
dnreg <- read.delim("../Results/DE_Downregulated.tsv")
head(knitr::kable(dnreg), 10)
```

Apart from this, we also made box plots, density plots and heatmaps to analyse the trends existing within our dataset. The plots are shown below: 
