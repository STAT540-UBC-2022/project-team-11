...
# To be sure there are no NAs
counts_pdata <- counts_pdata %>% 
                drop_na()

counts_pdata_pca <- prcomp(counts_pdata[,-c(1, 2, 3, 4, 5, 6)], 
                                            center = TRUE,
                                            scale. = TRUE)
...
ggbiplot(counts_pdata_pca, 
         ellipse = TRUE, 
         labels = rownames(counts_pdata),
         obs.scale = 1,
         var.scale = 1,
         groups = counts_pdata$Gender) +
