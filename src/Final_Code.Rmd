...
up_1 <- deg[deg$logFC > 0,]
down_1 <- deg[deg$logFC < 0,]
...
par(mfrow=c(1,2))
plotMD(lrt1,coef=1,status=de1[,"Sars_testpos:Age_categoryYoung Adult"], values = c(-1, 1), hl.col=c("blue","red"))
...
up_2 <- deg[deg$logFC > 0,]
down_2 <- deg[deg$logFC < 0,]
...
up_3 <- deg[deg$logFC > 0,]
down_3 <- deg[deg$logFC < 0,]
