...
designMatrix = model.matrix(~Sars_test, data = pdata_imputed)
...
detags <- rownames(dge_disp)[as.logical(de)]
plotMD(lrt, main="MD plot")
...
# Data prep 
```{r}
## Using top 100 genes irrespective of significance 
deg3_mod <- topTags(lrt3, n = 100, sort.by = "logFC")$table
...
# PCA analysis for possible dimension reduction
```{r}
# Perform PCA
...
### Data spliting for ml training and validation sets 
```{r}
set.seed(123)
...
## KNN model 
```{r}
## data prep: converting the predictor variable to numeric 
data_ml_train1 <- data_ml_train %>% 
  mutate(status = as.numeric(case_when(
  Sars_test == "pos" ~ "1",
  Sars_test == "neg" ~ "0")))

data_ml_validate1 <- data_ml_validate %>% 
  mutate(status = as.numeric(case_when(
  Sars_test == "pos" ~ "1",
  Sars_test == "neg" ~ "0")))

data_ml_train1 <- data_ml_train1[,-3]
data_ml_validate1 <- data_ml_validate1[,-3]

## data prep: converting the explanatory  variables to numeric for feature scaling
data_ml_train1$Age_category <- as.numeric(data_ml_train1$Age_category)
data_ml_validate1$Age_category <- as.numeric(data_ml_validate1$Age_category)

data_ml_train1$Gender <- as.numeric(data_ml_train1$Gender)
data_ml_validate1$Gender <- as.numeric(data_ml_validate1$Gender)

## Feature scaling for KNN
data_ml_train_scale <- scale(data_ml_train1[, -103])
data_ml_validate_scale <- scale(data_ml_validate1[, -103])
data_ml_validate_sub <- data_ml_validate_scale[ , colSums(is.na(data_ml_validate_scale)) == 0]
data_ml_train_sub <- data_ml_train_scale[,c(colnames(data_ml_validate_sub))]
...
for (i in seq_along(k_to_try)) {
  classifier_knn <- knn(train = data_ml_train_sub,
                      test = data_ml_validate_sub,
                      cl = data_ml_train1$status,
                      k = k_to_try[i])
  err_k[i] = calc_class_err(data_ml_validate1$status, classifier_knn)
}
...
## Using KNN with k=15 to avoid over fitting and yet having a small error
classifier_knn <- knn(train = data_ml_train_sub,
                      test = data_ml_validate_sub,
                      cl = data_ml_train1$status,
                      k = 15)
...
## Confusion matrix
cm <- table(data_ml_validate1$status, classifier_knn)
cm
...
data_ml_train_mod <- data_ml_train %>% 
  mutate(Sars_test = as.factor(case_when(
  Sars_test == "pos" ~ "1",
  Sars_test == "neg" ~ "0"
))) %>% 
dplyr::rename(status = Sars_test)

data_ml_validate_mod <- data_ml_validate %>% 
  mutate(Sars_test = as.factor(case_when(
  Sars_test == "pos" ~ "1",
  Sars_test == "neg" ~ "0"
))) %>% 
...
null <- glm(status ~ 1, family = binomial(link = 'logit'), data = data_ml_train_mod)
full <- glm(status ~ .,family = binomial(link = "logit"), data = data_ml_train_mod)

# Step wise regression
null_full <- stepAIC(null, scope=list(lower=null, upper=full), direction="both", trace = FALSE)
#summary(null_full)

## Training the classifier with top 3 significant genes 
classifier <- glm(status ~ EFS + MTCO2P22 + EXOC5P1, data = data_ml_train_mod, family = binomial(link = "logit"))

summary(classifier) 
## Prediction on the validation set
prob_pred = predict(classifier, type = 'response', newdata = data_ml_validate[-3]) ## Will return a vector of predicted probabilities of belong to infected (1) vs uninfected (0)

## Since this is a classification problem, we do not want probabilities but instead either 1 or 0.
classifier_logistic = ifelse(prob_pred > 0.5, 1, 0) ## Will round the probabilities to 1 or 0

## Checking how accurate our model is
cm = table(data_ml_validate_mod[, 3], classifier_logistic) ## Will return a confusion matrix. We notice that 1 COVID 19 patient was misclassified as healthy and 3 healthy patients were misclassified as diseased.
cm
