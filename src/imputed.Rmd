...
# Run goseq analysis
goseq_results <- goseq::goseq(pwf_deg_1,"hg19","geneSymbol", test.cats = c("GO:BP"), method = "Wallenius", use_genes_without_cat = TRUE)
...
# Run goseq analysis
goseq_results2 <- goseq::goseq(pwf_deg_2,"hg19","geneSymbol", test.cats = c("GO:BP"), method = "Wallenius", use_genes_without_cat = TRUE)
...
# Run goseq analysis
goseq_results3 <- goseq::goseq(pwf_deg_3,"hg19","geneSymbol", test.cats = c("GO:BP"), method = "Wallenius", use_genes_without_cat = TRUE)
...
```{r}
set.seed(123)
split = sample.split(PCA_data_subset$status, SplitRatio = 0.80) 
